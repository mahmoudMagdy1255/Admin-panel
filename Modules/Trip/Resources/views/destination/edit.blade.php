...
<section class="content">
    ...
    <div class="box box-info">
        ...
        @endforeach @endif
        <!-- /.box-header -->
        <form class="form-horizontal" action="{{url('/admin-panel/destination')}}/{{ $destination->id }}" method="POST" enctype="multipart/form-data">
            {{ csrf_field() }}
            {{ method_field('PUT') }}
                        ...
                        <div class="tab-content">
                            ...
                            <div class="tab-pane @if($loop->first) active @endif" id="{{ $lang->display_lang }}">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-8">
                                        <input type="text" autocomplete="off" class="form-control" value="{{ ValueOf($destination, $lang, 'title') }}" name="{{$lang->lang}}[title]" @if($loop->first) required @endif>
                                    </div>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-8">
                                        <textarea id="editor{{ $lang->id }}" name="{{$lang->lang}}[description]" style="width: 100%; height: 200px; font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;">{{ ValueOf($destination, $lang, 'description') }}</textarea>
                                    </div>
                                </div>
                            </div>
                            @endforeach
                    ...
                    <div class="col-sm-8">
                        <select class="form-control" name="parent_id">
                            ...
                            @foreach($destinations as $dest)
                                <option {{$dest->id == $destination->parent_id ? 'selected' : ''}} value="{{ $dest->id }}">{{ $dest->title }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-4">
                        ...
                        @if($destination->photo)
                            <img src="{{asset('public/images/destination/' . $destination->photo)}}" height="70" width="100">
                        @else
